...
async fn tool_shell_exec(
    ...
    let raw_command = input["command"]
        .as_str()
        .ok_or("Missing 'command' parameter")?;
    // Sanitize shell command: remove null bytes as defense-in-depth
    let command = raw_command.replace('\0', "");
...
mod tests {
    ...
    async fn test_capability_enforcement_aliased_tool_name() {
        ...
        let result = execute_tool(
            ...
            &serde_json::json!({"path": "/tmp/openfang_test_alias.txt", "content": "hello"}),
            None,
            Some(&allowed),
            None,
            None,

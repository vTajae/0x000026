...
pub struct AgentRouter {
    ...
    /// Name of the current default agent (for re-resolution on stale UUIDs).
...
impl AgentRouter {
    ...
    /// Update the default agent (thread-safe, no &mut self needed).
    pub fn update_default(&self, agent_id: AgentId) {
        // We can't mutate default_agent without &mut self, so update the name cache
        // and user defaults to point to the new agent.
        if let Some(name) = self.default_name.lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
            self.agent_name_cache.insert(name.clone(), agent_id);
        }
    }

    /// Get the name of the default agent (for stale UUID re-resolution).
    ...
    /// Set the default agent name (used during initial registration).
    pub fn set_default_name(&self, name: String) {
        *self.default_name.lock().unwrap_or_else(|e| e.into_inner()) = Some(name);
    }
